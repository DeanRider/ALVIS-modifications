...
public class ContigAlignmentDiagram 
{
    ...
    public ContigAlignmentDiagram(DetailedAlignmentFile alignmentFile, DiagramOptions options)
    {
        ...
        ChimeraFilter chimeraFilter = new ChimeraFilter(options.getMinChimeraCoverageProp(), options.getMinChimeraAlignmentProp());
    ...
    public void writeOutputFile(DiagramOptions options)
    {
        ...
        if(m_constantKey || m_detailedDiagram)
        {
            ...
            int refNumber = m_refNames.size(); // FIRST color generator listings
    ...
    void drawContig(ArrayList<DetailedAlignment> alignments, int x, int y)
    {
        ...
        m_contigDrawLength = contigLength / 3 + 500 ; // added by dean to scale the length of box for read length
        System.out.println(m_contigDrawLength);
        ...
        for(int i = 0; i < alignments.size(); i++)
        {
            ...
            String boxName = alignment.getTargetName(); // added by dean recently removed s.get(0) from alignments.get(0) to update each target name
            String orientation = alignment.isReverseAlignment() ? "-" : "+"; // added by dean
            ...
            m_drawer.drawText(x + start + 5, y + 3 + (i * alignmentHeight), boxName + " (" + orientation + ")", Drawer.Anchor.ANCHOR_LEFT, Drawer.Align.ALIGN_LEFT, "black"); /// added by dean for info inside boxes
        }
...
m_contigDrawLength = 2000 ; // added by dean
        int i = 0;
        int j = -1;
		...
		System.out.println(name); // added by dean is the name of the matching chunk like NC00001.11
    ...
    private void drawContigAlignmentPage(ArrayList<String> queryNames, LinkedHashSet<String> refNames)
    {
        ...
        {             
            ...
            m_coloursMap.clear();  // SECOND color generator listings
            ColourGenerator colourGenerator = new ColourGenerator(refNames.size(), 0.7f, 0.7f);
		System.out.println(refNames.size()); // added by dean
        ...
        if(m_drawer instanceof SVGDrawer)
        {
			...
			System.out.println(alignment);  // added by dean
